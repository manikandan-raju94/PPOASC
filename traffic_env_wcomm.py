...
# ===================================================================================
    ...
    def _get_context_weights(self):
        ...
        raw_nci = avg_queue / 80.0
        self.nci = 0.9 * getattr(self, "nci", 0.0) + 0.1 * raw_nci
        nci = np.clip(self.nci, 0.0, 1.0)

        weights = {
            "queue": -0.15 - 0.2 * nci,
            "pressure": -0.08 - 0.15 * nci,
            "spillback": -0.08 - 0.2 * nci,
            "wait": -0.08 - 0.1 * nci,

            # 🔥 STRONG FLOW (CRITICAL FIX)
            "flow": 2.0 + 2.0 * nci
        }
    ...
    def _compute_reward(self):
        ...
        avg_pressure = np.mean(self.pressure_j) / 30.0
        avg_spill = np.mean(self.spill_j) / 5.0
        ...
        # =====================================================
        # 2. Throughput (FIXED SCALE)
        # =====================================================
        new_completed = self.completed - self.prev_completed
        self.prev_completed = self.completed

        flow = (new_completed / DELTA_TIME) / 3.0

        # =====================================================
        # 3. Get weights (ONLY FROM HERE)
        # =====================================================
        ...
        for lane in self.lane_list:
            try:
                if traci.lane.getLastStepMeanSpeed(lane) < ETC_SPEED_DROP:
                    slow_ratio += 1
            except:
                pass

        slow_ratio /= max(len(self.lane_list), 1)
        reward -= 0.05 * slow_ratio

        transition_pen = sum(
            1 for j in self.junctions
            if self.phase_manager.is_transitioning(j)
        ) / max(self.num_agents, 1)

        reward -= 0.05 * transition_pen

        # =====================================================
        # 6. Stabilization
        # =====================================================
        reward += 0.1   # stronger bias
        reward = np.clip(reward, -5, 5) / 5.0
